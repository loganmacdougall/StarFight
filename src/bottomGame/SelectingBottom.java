...
public class SelectingBottom {
	...
	{	
		...
		{
			O.drawButton(g, J, Game.getP1().getJavaColor());
		}
		//Sets the color of the text to the same as P1's ship.
		g.setColor(Game.getP1().getJavaColor());
		...
		for(int i = 0;i<P1Moves.length;i++)
		{
			...
			//Draw each button.
			Moves[i].drawButton(g, J, Game.getP1().getJavaColor());
		}
	}
	//Draws P1's moves to the screen
	...
	{	
		...
		for(Button O : options)
		{
			O.drawButton(g, J, Game.getP2().getJavaColor());
		}
		//Sets the color of everything being drawn.
		g.setColor(Game.getP2().getJavaColor());
		...
		for(int i = 0;i<P2Moves.length;i++)
		{
			...
			//Draw each button.
			Moves[i].drawButton(g, J, Game.getP2().getJavaColor());
