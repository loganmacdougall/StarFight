...
public class Ship {
	...
	// Stores the rotation of the Ship (0 = up, 90 = right, 180 = down & 270 =
	// left).
	...
	public Ship(String color, int x, int y) {
		setupShip(
				Optional.of(color),
				Optional.empty(),
				Optional.of((double) x),
				Optional.of((double) y),
				Optional.empty());
	}
	...
	private void setupShip(Optional<String> colorO, Optional<Double> scaleO, Optional<Double> xO, Optional<Double> yO,
			Optional<Double> rotationO) {
		this.color = colorO.isPresent() ? colorO.get() : "B&W";
		this.colorNum = getColorNum(this.color);

		String ShipImageFilePath = "res/" + getColor() + " Ship.png";
		try {
			File shipImageFile = new File(ShipImageFilePath);
			this.ship = ImageIO.read(shipImageFile);
		} catch (IOException e) {
			System.err.println("Error: failed to read image \"" + ShipImageFilePath + "\"");
		}
		this.scale = scaleO.isPresent() ? scaleO.get() : 20.0;
		this.x = xO.isPresent() ? xO.get() : 0.0;
		this.y = yO.isPresent() ? yO.get() : 0.0;
		this.rotation = rotationO.isPresent() ? rotationO.get() : 0.0;
	}
	...
	public int getColorNum(String color) {
		if (color.equalsIgnoreCase("B&W") || color.equalsIgnoreCase("Black & White")
				|| color.equalsIgnoreCase("Black and White")) {
			return 0;
	...
	public BufferedImage getShip(int colorNum) {
		try {
			return ImageIO.read(new File("res/" + getColor(colorNum) + " Ship.png"));
		} catch (IOException e) {
		}
		return ship;
	}
	...
	public BufferedImage getShip(String color) {
		try {
			return ImageIO.read(new File("res/" + color + " Ship.png"));
		} catch (IOException e) {
		}
		return ship;
	}
	...
	public void setShip() {
		try {
			this.ship = ImageIO.read(new File("res/" + getColor() + " Ship.png"));
		} catch (IOException e) {
		}
	}
	...
	public Laser shoot() {
		return new Laser(getColor(), getScale(),
				(int) (getX() + Math.sin(Math.PI * getRotation() / 180) * 11.5 * getScale()),
				(int) (getY() - Math.cos(Math.PI * getRotation() / 180) * 11.5 * getScale()), getRotationInt());
	}
		...
		if (getBurnedAmount() > 0) {
			...
			try {
				if (getBurnedAmount() % 40 <= 13) {
					g2d.drawImage(ImageIO.read(new File("res/Fire1.png")), atF, F);
				} else if (getBurnedAmount() % 40 <= 26) {
					g2d.drawImage(ImageIO.read(new File("res/Fire2.png")), atF, F);
				} else if (getBurnedAmount() % 40 <= 39) {
					g2d.drawImage(ImageIO.read(new File("res/Fire3.png")), atF, F);
				}
			} catch (IOException e) {
			}
		}
