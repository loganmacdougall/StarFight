...
public class Laser {
	...
	// Stores the rotation of the Laser (0 = up, 90 = right, 180 = down & 270 =
	// left).
	...
	private void setupLaser(Optional<String> colorO, Optional<Double> scaleO, Optional<Double> xO, Optional<Double> yO,
			Optional<Double> rotationO) {
		this.color = colorO.isPresent() ? colorO.get() : "B&W";
		this.colorNum = getColorNum(this.color);

		String laserImageFilePath = "res/" + getColor() + " Laser.png";
		try {
			File laserFile = new File(laserImageFilePath);
			this.laser = ImageIO.read(laserFile);
		} catch (IOException e) {
			System.err.println("Error: failed to read image \"" + laserImageFilePath + "\"");
		}

		this.scale = scaleO.isPresent() ? scaleO.get() : 20.0;
		this.x = xO.isPresent() ? xO.get() : 0;
		this.y = yO.isPresent() ? yO.get() : 0;
		this.rotation = rotationO.isPresent() ? rotationO.get() : 0;
	}
	...
	public void setLaser() {
		try {
			this.laser = ImageIO.read(new File("res/" + getColor() + " Laser.png"));
		} catch (IOException e) {
		}
	}
